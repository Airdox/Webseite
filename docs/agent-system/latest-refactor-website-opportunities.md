...
Generated: 2026-05-31T16:30:31.833Z
Agent: Refactor
...
| medium | maintainability | large-file-src-server-worker-js | 685 lines / 24935 bytes. | Extract only the next user-visible or service-flow subunit that can receive a focused test. | Run the nearest component/unit test plus npm run build. | src/server/worker.js |
...
| medium | maintainability | large-file-src-components-setcard-jsx | 384 lines / 16744 bytes. | Extract only the next user-visible or service-flow subunit that can receive a focused test. | Run the nearest component/unit test plus npm run build. | src/components/SetCard.jsx |
